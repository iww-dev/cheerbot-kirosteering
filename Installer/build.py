...
class Builder:
    ...
    def header(self):
        ...
        print(f"  ▸ Cheerbot Builder v0.0.2 ◂")
    ...
    def build_executable(self):
        ...
        try:
            ...
            if result.returncode == 0:
                exe_path = self.dist_path / "CheerbotInstaller.exe"
                if exe_path.exists():
    ...
    def build_executable_fallback(self):
        try:
            ...
            if result.returncode == 0:
                exe_path = self.base_path / "dist" / "CheerbotInstaller.exe"
                if exe_path.exists():
                    size_mb = exe_path.stat().st_size / (1024 * 1024)
                    ...
                    if exe_path != self.dist_path / "CheerbotInstaller.exe":
                        self.dist_path.mkdir(exist_ok=True)
                        shutil.move(str(exe_path), str(self.dist_path / "CheerbotInstaller.exe"))
    ...
    def create_launcher(self):
        ...
        try:
            launcher_path = self.base_path / "CheerbotInstaller.bat"
            launcher_content = f"""@echo off
cd /d "{self.dist_path}"
CheerbotInstaller.exe
pause
"""
    ...
    def create_readme(self):
        ...
        try:
            ...
            readme_content = f"""Cheerbot Installer Build Info
{'=' * 40}

Build Date: {__import__('datetime').datetime.now().strftime('%Y-%m-%d %H:%M:%S')}
Python Version: {sys.version.split()[0]}
Executable: {self.dist_path / 'CheerbotInstaller.exe'}

Quick Start:
1. Run CheerbotInstaller.exe
2. Follow the installation wizard
3. Steering rule will be installed to:
   C:\\Users\\[YourUsername]\\.kiro\\steering\\cheerbot.md

For more info visit:
https://github.com/warwakei/cheerbot-kirosteering
"""
    ...
    def show_summary(self):
        ...
        recommended_path = self.base_path / "cheerbot-ext1.md"
